...
public class AdminControllerUsers {
	...
	public String createEmployee(
	        @Validated @ModelAttribute(DefineAttribute.UserBeanAttribute) UserBean user,
	    ...
	    if (user.getAvatar() != null && !user.getAvatar().isEmpty()) {
	        ...
	        File uploadDir = new File(accountImgDir.getPath());
	        if (!uploadDir.exists()) {
	            uploadDir.mkdirs();
	        }

	        avatarFileName = user.getAvatar().getOriginalFilename();
	        File file = new File(uploadDir.getPath() + File.separator + avatarFileName);
	        ...
	        try {
	            user.getAvatar().transferTo(file);
	        } catch (Exception e) {
	            e.printStackTrace();
	    ...
	    if (!user.getAvatarDir().isEmpty()) {
	        account.setAvatar(user.getAvatarDir()); // Ưu tiên đường dẫn chỉ định nếu có
	    }
