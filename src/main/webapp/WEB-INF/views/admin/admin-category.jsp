						...
						<div class="col-md-12">
							<div class="d-md-flex justify-content-between align-items-center">
								...
								<div>
									...
									<!-- Nut undo chi hien thi khi undo duoc -->
        							<c:if test="${not empty sessionScope.lastDeletedCategoryMemento}">
            							<a href="admin/category/undo-delete.htm" 
               								class="btn btn-outline-secondary"
               								onclick="return confirm('Are you sure you want to undo the last deletion?')">
               								<i class="fas fa-undo me-1"></i> Undo Delete
            							</a>
        							</c:if>
								</div>
							</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Thêm confirm dialog cho nút Undo
    const undoBtn = document.querySelector('.alert-success a[href*="undo-delete"]');
    if (undoBtn) {
        undoBtn.addEventListener('click', function(e) {
            if (!confirm('Are you sure you want to undo the last deletion?')) {
                e.preventDefault();
            }
        });
    }
